...
const Login = () => {
  ...
  const onSubmitHandler=async(event)=>{
    event.preventDefault() //not reload the web page on submitting the form
    ...
    <form onSubmit={onSubmitHandler} className="min-h-[80vh] flex items-center">
      <div className="flex flex-col gap-3 m-auto items-start p-8 min-w-[340px] sm:min-w-96 rounded-xl text-zinc-600 text-sm shadow-lg">
        ...
        <button type="submit" className="bg-[#5f6FFF] text-white w-full py-2 rounded-md text-base">{state==='Sign Up' ? "Create Account" : "Login"}</button>
        {
          ...
          : <p>To Create a new account: <span onClick={()=>setState('Sign Up')} className="text-[#5f6FFF] underline cursor-pointer">Click Here</span></p>
        }
      </div>
